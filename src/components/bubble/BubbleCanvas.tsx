...
import type { Bubble as BubbleType, Song, CategoryFilterValue, BubbleType as BubbleTypeEnum } from '../../types'
...
const generateBubbleSources = (songs: Song[]): BubbleSource[] => {
  ...
  songs.forEach((song) => {
    sources.push({
      ...
      type: 'song',
      name: song.title,
      relatedCount: (song.lyricists?.length || 0) + 
...
export const BubbleCanvas: React.FC<BubbleCanvasProps> = React.memo(({
  ...
  const filteredSources = useMemo(() => {
    ...
    return bubbleSources.filter(source => 
      matchesCategoryFilter(source.type, categoryFilter)
    )
